...
#undef JPGAL_FUNC
#define JPGAL_FUNC "write_quantization_table"
{
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "setup_huffman_tree"
{
	HUFFMAN_NODE *tail;
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "find_leaf_at_level"
{
	HUFFMAN_NODE *result;
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "destroy_huffman_tree"
{
	if (!node)
		return;
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "compare_entries"
{
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "build_huffman_table"
{
	HUFFMAN_TREE tree;
	...
	TRACE("Exchanging rightmost leaf at level %d with fake leaf at level %d",
		node->entry->bits_length, fake_entry->bits_length);
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "write_huffman_table"
{
	HUFFMAN_ENTRY *entry;
	int i, j, code, index;
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "write_header"
{
	char *comment = JPGALLEG_VERSION_STRING;
	unsigned int i;
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "format_number"
{
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "huffman_encode"
{
	HUFFMAN_ENTRY *entry;
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "encode_block"
{
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "_jpeg_c_rgb2ycbcr"
{
	int r, g, b;
 ...
 */
static int
encode_pass(BITMAP *bmp, AL_CONST RGB *pal, int quality, int flags)
#undef JPGAL_FUNC
#define JPGAL_FUNC "encode_pass"
{
	...
	TRACE("Saving %dx%d %s image in %s mode%s", bmp->w, bmp->h, greyscale ? "greyscale" : "color",
		sampling == JPG_SAMPLING_444 ? "444" : (sampling == JPG_SAMPLING_422 ? "422" : "411"),
				...
				for (block_y = 0; block_y < bmp->h; block_y += 16) {
					for (block_x = 0; block_x < bmp->w; block_x += 16) {
						...
						if (encode_block(y1, LUMINANCE, &dc_y) ||
								...
								encode_block(y3, LUMINANCE, &dc_y) ||
								encode_block(y4, LUMINANCE, &dc_y) ||
								encode_block(cb, CHROMINANCE, &dc_cb) ||
								encode_block(cr, CHROMINANCE, &dc_cr))
							goto exit_error;
					}
				}
				break;
...
#undef JPGAL_FUNC
#define JPGAL_FUNC "_jpeg_encode"
{
	IO_VTABLE old_io;
	...
	if (flags & JPG_OPTIMIZE) {
		...
		old_io = _jpeg_io;
		_jpeg_io = _jpeg_dummy_io;
		current_pass = PASS_COMPUTE_HUFFMAN;
		result = encode_pass(bmp, pal, quality, flags);
		if (result) {
			TRACE("First pass failed");
			goto exit_error;
		}
		result |= build_huffman_table(&huffman_dc_table[0], num_codes_dc_luminance, val_dc_luminance);
		result |= build_huffman_table(&huffman_ac_table[0], num_codes_ac_luminance, val_ac_luminance);
		if (!(flags & JPG_GREYSCALE)) {
			result |= build_huffman_table(&huffman_dc_table[1], num_codes_dc_chrominance, val_dc_chrominance);
			result |= build_huffman_table(&huffman_ac_table[1], num_codes_ac_chrominance, val_ac_chrominance);
		}
		if (result) {
